        ...
        <div className="mb-5">
          ...
          <div className="flex justify-between">
            <ul className="flex items-center">
              <li
                className={`${
                  selectedCategory === "All" ? "filter-active bg-primary" : ""
                } text-white px-5 py-2 rounded-full font-DmSans font-semibold text-lg`}
              ...
              {discoverData.discover.category.map((category: any, i: number) => {
                return (
                  <li
                    key={i}
                    className={`${
                      selectedCategory === category.title ? "filter-active " : ""
                    } text-black bg-grayPrimaryLight px-6 py- rounded-full font-DmSans font-semibold text-lg ml-3`}
            ...
            <div>
              <button className="flex font-DmSans text-primary font-semibold bg-grayPrimaryLight px-5 py-3 text-lg rounded-full">
                <img src="/images/icon/filter.svg" alt="" className="mr-1" />
                All Filters
              </button>
            </div>
          </div>
          ...
          <div className="grid lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 gap-10 mt-10">
            {discover.items
              .filter((item: any) => (selectedCategory === "All" ? true : item.item_cate === selectedCategory))
              .slice(0, numToShow)
                ...
                return (
                  ...
                  /> */}
                    <div className="max-w-sm rounded-xl overflow-hidden shadow-sm bg-white p-3 ">
                      ...
                      <div className="flex flex-shrink-0 absolute -mt-6 ml-3 discover flex-wrap">
                        {item.author?.map((author: any, i: number) => (
                          <img key={i} src={author.avater} alt="" />
                        ))}
                      </div>
                      <div className="p-2 mt-4">
                        ...
                        <div className="flex justify-between mb-5 pb-5 border-b border-b-slate-100">
                          <span className="flex items-center text-green-600 text-sm font-DmSans font-semibold">
                            <FaEthereum className="text-lg mr-1" />
                            {item.amount}
                          </span>
                        ...
                        <div className="flex justify-between items-center pt-1">
                          ...
                          <Link href="#" className="text-primary text-md font-DmSans hover:border-b">
                            {item.btn}
                          </Link>
                        </div>
                      </div>
